...
int argc;
char **argv;
{   int tcpskt,udpskt,sleepflg;
    int incudpcnt=0,prvincudpcnt=0,inctcpcnt=0,prvinctcpcnt=0;
    struct sockaddr_in remtcpadr,remudpadr;
    char *buf;
    fd_set readfds;
    ...
    {   for(sleepflg=0;;sleepflg=1)
        {   if(sleepflg)
                sleep(RETRYSLEEP);
            if((tcpskt=opnsock(SOCK_STREAM,0,0))>=0)
            {   if((udpskt=opnsock(SOCK_DGRAM,1,
                ...
                {   memset(&remudpadr,0,sizeof(struct sockaddr_in));
                    ...
                    {   remudpadr.sin_addr.s_addr=*((uint32_t *)
                        ...
                        {   memset(&remtcpadr,0,sizeof(struct sockaddr_in));
                            remtcpadr.sin_family=AF_INET;
                            remtcpadr.sin_port=htons(argbuf.arg_remtcprt);
                            if((phostent=gethostbyname(argbuf.arg_remtcphst)))
                            {   remtcpadr.sin_addr.s_addr=*((uint32_t *)
                                    (phostent->h_addr_list[0]));
                                if(!connect(tcpskt,(struct sockaddr *)
                                    (&remtcpadr),sizeof(struct sockaddr_in)))
                                {   for(;;)
                                    {   FD_ZERO(&readfds);
...
{   printf("\n%s [-u (%3s)]",argbuf->arg_prgnam,
    ...
    printf(" <tieudptnlc udp port>\n");
    printf("                       ");
    printf(" <tieudptnl tcp host > <tieudptnl tcp port>\n");
    printf("                       ");
...
{   argbuf->arg_prgnam=argv[0];
    ...
    argbuf->arg_locudprt=0;
    argbuf->arg_remtcphst="";
    argbuf->arg_remtcprt=0;
